...
namespace Grpahs.Structures
{
    public class EdgeList
    {
        ...
        {
            list.Remove(list.Find(x => x.Vertexes.Contains(from) && x.Vertexes.Contains(to)));
        }

        public Edge GetLink(Vertex from, Vertex to)
        {
            return list.Find(x => x.Vertexes.Contains(from) && x.Vertexes.Contains(to)).Edge;
        }

        public bool IsLinked(Vertex from, Vertex to)
        {
            return list.FirstOrDefault(x => x.Vertexes.Contains(from) && x.Vertexes.Contains(to)) != null;
        }
        ...
        public List<Vertex> GetLinks(Vertex vertex)
        {
            ...
            {
                if(node.Vertexes.Contains(vertex))
                    links.Add(node.Vertexes.First(x => x != vertex));
            }
            return links;
        }
    }
